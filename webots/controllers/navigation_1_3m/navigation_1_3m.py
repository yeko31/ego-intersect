...
def save_camera_image(camera, folder, counter):
    ...
    if image:
        ...
        filename = os.path.join(folder, f"a130cm_env1_ms_50_{counter:06d}.jpg")
...
if __name__ == '__main__':
    ...
    # ================== WAYPOINT NAVIGATION ==================
    waypoints = [
        (-3.39,-2.14,FLYING_ATTITUDE),#0
        (1, -2.14, FLYING_ATTITUDE),
        (5.91, -2.2, FLYING_ATTITUDE),
        (5.94, -0.19, FLYING_ATTITUDE),
        (1.11, -0.17, FLYING_ATTITUDE),
        (-3.69, -0.12, FLYING_ATTITUDE),
        (-3.69, 1.66, FLYING_ATTITUDE),
        (1.06, 1.72, FLYING_ATTITUDE),
        (5.81,1.77, FLYING_ATTITUDE),
        (5.84 ,3.65, FLYING_ATTITUDE),
        (1.12 ,3.66, FLYING_ATTITUDE),
        (-3.1 ,3.89, FLYING_ATTITUDE),
        (1.21 ,3.73, FLYING_ATTITUDE),
        (5.84 ,3.65, FLYING_ATTITUDE),
        (5.81,1.77, FLYING_ATTITUDE),
        (1.06, 1.72, FLYING_ATTITUDE),
        (-3.69, 1.66, FLYING_ATTITUDE),
    ...
    # Rotation config
    yaw_rotation_points = {
        2: {'direction': +1, 'cycles': 80},   # (5.91, -2.2)
        3: {'direction': +1, 'cycles': 80},   # (5.94, -0.19)
            ...
            # Case 2: INSIDE save_distance_before_wp → trigger wiggle
                ...
                if current_wp_index in straight_buffer and len(straight_buffer[current_wp_index]) > 0:
                    ...
                    for cnt, img_bgr in last_five:
                        filename = os.path.join(STRAIGHT_DIR, f"a130cm_env1_ms_50{cnt:06d}.jpg")
                        cv2.imwrite(filename, img_bgr)
